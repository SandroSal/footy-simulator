...
**Open:**
- [ ] **Classifier reliability and stability at cheap-tier sizes.** Two coupled properties to validate in prototype. (1) *Routing accuracy*: classification of recognized-intent (cheap path) vs. novel-intent (expensive path) must be reliable at small model sizes. Misroute too aggressively to expensive, costs blow up; too aggressively to cheap, Pillar 1 becomes fake (system silently flattens player input into menu options). (2) *Same-instruction consistency*: the same instruction issued twice should produce similar tasks (same role stat, similar difficulty) but different rolls — otherwise the system feels arbitrary rather than reasoned. Load-bearing prototype risk for the whole architecture — if either property fails at small model sizes, the cost gap collapses and §10.5 has to be revisited. Need explicit classifier evals from first prototype. Now evaluable against a defined scope (the §3 recognized-intent list and supported-domain list).
