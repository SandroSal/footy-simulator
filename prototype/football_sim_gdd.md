...
The slot-consumption axis (instant vs. strategic) is orthogonal to the classifier-routing axis (recognized vs. novel — see §3). Both axes need stating because they answer different questions.

- **Instant actions** do *not* consume a slot. These include menu actions and any chat instruction the classifier identifies as a *routine recognized intent* (per §3) — e.g., "fire the CD," "accept the kit sponsor offer," "how much cash do we have?" Officer handles them immediately — fiction is "a quick phone call the officer can sign off on."
- **Strategic tasks** consume the active slot. These include novel chat instructions, event responses, *and* recognized intents the classifier identifies as strategic (e.g., "renegotiate Smith's contract," "bid €5m for Smith," "handle the press on the Jones scandal") — i.e., structured enough to classify cheaply, but the work itself takes real time. All strategic paths run the §3 outcome pipeline.
...
3. **Event responses (LLM).** Responses to surfaced events route through the same classifier as chat instructions (the `{active_events}` context allows stage-1 to disambiguate). Most event responses resolve to a strategic recognized intent (e.g. `crisis_response` for crisis events, `sponsorship_negotiation_named` for sponsor overtures, `transfer_initiation_sell` for player-driven transfer requests), some resolve to novel; either way they consume a slot and run the full LLM pipeline. The "event response" surface is a UX distinction (the player is replying to a surfaced card with a known event_id in context), not a separate pipeline.
...
For any strategic path — novel intents, recognized strategic intents, and event responses (which classify to one of the previous two):
...
- *2026-05-16 — §2/§3 doc consistency fix: clarified that instant-vs-strategic and recognized-vs-novel are orthogonal axes, and that event responses route through the classifier rather than skipping it.* Two related inconsistencies surfaced during a doc read. (1) The §2 "Instant vs. strategic" subsection conflated recognized = instant and novel = strategic. The refined model in §3 ("What's not on the menu") and the stage-1 prompt both treat the two axes as orthogonal: recognized intents split into routine (instant) and strategic (consumes slot). §2 now states the orthogonality explicitly with examples of strategic recognized intents. (2) The §3 interaction-model entry for event responses described them as going "through the full LLM pipeline as a strategic task" without mentioning the classifier, which read as "event responses skip stage 1." The stage-1 prompt has `{active_events}` in context and an event-bound `crisis_response` recognized intent, so event responses do classify; some land on recognized strategic intents (crisis_response, sponsorship_negotiation_named, transfer_initiation_sell), some on novel. The §3 fix reframes "event response" as a UX surface, not a pipeline step. The §3 pipeline preamble was also tightened to enumerate the three strategic entry paths. No mechanical change — the classifier and slot model already work this way per the stage-1 prompt; this is a doc-consistency fix.

- *2026-05-16 — §2 Task Slot Model "Instant vs. strategic" subsection rewritten to align with the refined classifier model in §3.* The original §2 passage conflated two axes: it described instant actions as "menu actions and routine/recognized intents" (treating recognized = routine) and strategic tasks as "novel chat instructions and event responses" (treating strategic = novel). §3's "What's not on the menu" subsection later refined this: recognized intents split into routine (instant) and strategic (consumes slot), so recognized vs. novel and instant vs. strategic are orthogonal axes. The §2 passage was never updated. The fix states the orthogonality explicitly and gives examples of strategic recognized intents (transfer bid, contract renegotiation) so the reader doesn't have to reconcile the two sections themselves. No mechanical change — the classifier and slot model already work this way per §3; this is a doc-consistency fix.
